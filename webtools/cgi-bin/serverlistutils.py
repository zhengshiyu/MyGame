...
def sethtmlFile(serverlist,serverdata):
	...
	if serverlist!="":
		...
		f.write(serverdata)
		f.close()
		commandsutils.execCmd("scp -i ~/.ssh/id_server_release "+tempdir+serverlist+" 168:/data0/wg_www/ss.ec.feidou.com/serverlist/res/html/"+serverlist)
def getAllArealist():
	stat,ret = commands.getstatusoutput('ssh 168 "cd /data0/wg_www/ss.ec.feidou.com/serverlist/ && ls  -F *.xml"')
	return ret
def getAreaListFile(serverlist):
	try:
		response = urllib2.urlopen("http://res.qyz.feidou.com/serverlist/arealist_"+serverlist+".xml"+"?"+str(datetime.datetime.now().microsecond))
		xmlstring = response.read()
		return xmlstring
	except urllib2.URLError, e:
		print "URLError"
...
def setServerlist(serverlist,serverdata):
	...
	if serverlist!="":
		...
		f.write(serverdata)
		f.close()
		commandsutils.execCmd("scp -i ~/.ssh/id_server_release "+tempdir+serverlist+" 168:/data0/wg_www/ss.ec.feidou.com/serverlist/"+serverlist)
